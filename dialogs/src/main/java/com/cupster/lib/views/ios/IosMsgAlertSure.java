...
public class IosMsgAlertSure extends AlertDialog implements OnClickListener {
    ...
    private boolean backForbid = false;

    public IosMsgAlertSure(Context context) {
        this(context, R.style.ios_dialog);
        mContext = context;
        mInflater = LayoutInflater.from(context);
    }

    public IosMsgAlertSure(Context context, int themeResId) {
...
//            p.height = (int) (d.getHeight() * 0.3); // 高度设置为屏幕的
    ...
    /**
     * 返回键 是否dismiss dialog
     * @param forbid
     */
    public void setBackForbid(boolean forbid){
        backForbid = forbid;
    }
    ...
    public void setMessageGravity(int gravity) {
        ...
        params.setMargins(dip2px(mContext, 30), dip2px(mContext, 10), dip2px(mContext, 20), dip2px(mContext, 10));
